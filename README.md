...
- places the game's own Poké Ball tile in the opponent row when that wild
  species has already been caught
- keeps both native level indicators: player status sits to its left on the
  same content grid, while opponent status sits to its right after the caught
  slot
...
FORK 1.7.6 and Dramaless Shape. In the classic layout the native HUD draw is
